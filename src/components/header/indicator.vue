<template>
    ...
    >
        <AppLink v-if="to && trigger !== 'click'" :to="to" class="indicator__button esta1" @click.native="onClick">
            <span class="indicator__area">
                <slot name="icon" />
                <span v-if="value !== null" class="indicator__value">{{ value }}</span>
            </span>
        </AppLink>
        <a v-if="to && trigger === 'click'" :href="$url.anyLink(to)" class="indicator__button esta 2" @click="onClick">
            <span class="indicator__area">
                <slot name="icon" />
                <span v-if="value !== null" class="indicator__value">{{ value }}</span>
            </span>
