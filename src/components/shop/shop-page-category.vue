...
export default class ShopPageCategory extends Vue {
    ...
    get pageTitle () {
        return this.category ? this.category.name : 'Tienda'
    }
    ...
    get breadcrumb (): ILink[] {
        ...
        if (this.category) {
            getCategoryParents(this.category).forEach((parent) => {
                breadcrumb.push({ icon: '/images/comedor.svg', title: parent.name, url: this.$url.category(parent) })
            })

            breadcrumb.push({ icon: '/images/comedor.svg', title: this.category.name, url: this.$url.category(this.category) })
        }
