...
@Component({
    ...
    head (this: ShopPageProduct) {
        return {
            ...
            script: [{ type: 'application/ld+json', json: this.structure }],
            meta:[
                {
                    hid: 'description',
                    name: 'description',
                    content:this.product.des
                },
                {
                    hid: 'keywords',
                    name: 'keywords',
                    content: this.product.keywords
                }
                ]
            }
        }
...
export default class ShopPageProduct extends Vue {
    ...
    structure:any= {
        ...
        "image":this.$url.img(this.product.images[0]),
        "description":this.product.des,
        "alternateName":this.product.slug,
        "aggregateRating": {
            "@type": "AggregateRating",
            "ratingValue": this.product.rating==0 ? "5":this.product.rating,
            "reviewCount": "1"
        },
        "offers": {
            "@type": "Offer",
    ...
    get breadcrumb (): ILink[] {
        return [
            { icon: '/images/comedor.svg', title: 'Home', url: this.$url.home() },
            { icon: '/images/comedor.svg', title: 'Tienda', url: this.$url.catalog() },
            { icon: '/images/comedor.svg', title: this.product.name, url: this.$url.product(this.product) },
