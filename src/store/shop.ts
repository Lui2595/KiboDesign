...
export const mutations: MutationTree<ShopState> = {
    ...
    setOptionValue (state, payload: SetOptionValuePayload) {
        ...
        state.options.pageUrl=payload.url as any
    },
...
export const actions: ActionTree<ShopState, {}> = {
    ...
    async fetchProductsList ({ commit, state },payload:any): Promise<void> {
        let canceled = false
        ...
        if(payload.search){
            productsList = await shopApi.search(payload.search)
        }else{
            productsList = await shopApi.getProductsList(state.options, filters)
        }
    ...
    async setOptionValue ({ dispatch, commit }, payload: SetOptionValuePayload) {
        ...
        //console.log(payload);
        await dispatch('fetchProductsList', payload)
    },
