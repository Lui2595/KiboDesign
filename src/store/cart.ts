...
export const mutations: MutationTree<CartState> = {
    add (state, payload: CartAddPayload) {
        ...
        const variantes=payload.product.variaciones;

        if(variantes.madera.length==0){
            Vue.notify({
                type: 'warning',
                text: `Por favor selecciona la madera`
            })
            return;
        }
        if(variantes.telas.length==0){
            Vue.notify({
                type: 'warning',
                text: `Por favor selecciona la tela`
            })
            return;
        }
        if(variantes.dimensiones.length==0){
            Vue.notify({
                type: 'warning',
                text: `Por favor selecciona la dimension`
            })
            return;
        }
        ...
        if (itemIndex === -1) {
            ...
            const variantesitem=item.product.variaciones
            // console.log(variantesitem);
            // console.log(variantes)
            //     alert(variantesitem)
            //     alert(variantes)
                if(JSON.stringify(variantesitem)!=JSON.stringify(variantes)){
                    state.items.push({
                        id: ++state.lastItemId,
                        product: JSON.parse(JSON.stringify(product)),
                        options: JSON.parse(JSON.stringify(options)),
        ...
        Vue.notify({
            type: 'success',
            text: `Producto  "${product.name}" agregado al carrito!`
        })
    },
    default(state){
...
export const actions: ActionTree<CartState, {}> = {
    ...
    async default ({ commit }): Promise<void> {
        await new Promise<void>((resolve) => {
            setTimeout(() => {
                resolve()
            }, 500)
